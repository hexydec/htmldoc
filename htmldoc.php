...
class htmldoc implements \ArrayAccess {
	 ...
	 * @param tag $parent The parent tag, or null if there is no parent
	 * @return bool Whether the parser was able to capture any objects
	 */
	public function parse(Array &$tokens, tag $parent = null) : bool {
		$parenttag = is_object($parent) ? $parent->tagName : null;
				...
				switch ($token['type']) {
					...
					case 'tagopenstart':
						...
						if ($tag == $parenttag && in_array($tag, $this->config['elements']['closeoptional'])) {
							...
							// parse the tag
							$item = new tag($tag, $this->config, $parent);
							$item->parse($tokens, $parent);
							$this->children[] = $item;

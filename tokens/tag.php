...
class tag {
	...
	public function __construct(string $tag, array $config, tag $parent = null) {
		$this->tagName = $tag;
		$this->config = $config;
	}
	 ...
	 */
	public function parse(Array &$tokens, tag $parent = null) {
		$this->parent = $parent;
		$attr = false;
			...
			switch ($token['type']) {
				...
				case 'tagopenend':
					if (!in_array($this->tagName, $this->config['elements']['singleton'])) {
						...
						$this->children->parse($tokens, $this);
					}
					break;
		...
		foreach ($selector AS $i => $item) {
			...
			if ($item['join'] && $i) {
				...
				if (($children = $this->children->find(Array(array_slice($selector, $i)))) !== false) {
					$found = array_merge($found, $children->get());
				}
				break;
			...
			// match pseudo selector
				...
				// match last child
					...
					if ($this !== $this->parent->children()->get(-1)) {
						$match = false;
						break;
					}
				}
